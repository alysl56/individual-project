...
map <- AnnotationDbi::select(org.Hs.eg.db, keys = res$gene_clean,
                             keytype = "ENSEMBL", columns = c("SYMBOL"))
res <- res %>% left_join(map, by = c("gene_clean" = "ENSEMBL"))
res$SYMBOL[is.na(res$SYMBOL) | res$SYMBOL==""] <- res$gene_clean
...
p_core <-
  ggplot(df, aes(x = log2FoldChange, y = neglog10padj, color = status)) +
  ...
  labs(
    ...
    title    = "Volcano plot",
    subtitle = "A549: with DMSO vs without DMSO"
  ) +
...
p_final <- cowplot::ggdraw(p_core) +
  cowplot::draw_label("padj < 0.05\n|log2FC| > 1",
             x = 0.78, y = 0.89, hjust = 0, vjust = 0,
             fontface = "plain", size = 10, color = "black",
             lineheight = 1.05)

dir.create(file.path(base,"DE_analysis"), showWarnings = FALSE, recursive = TRUE)
ggsave(file.path(base,"DE_analysis","A549_volcano.pdf"), p_final, width = 6.6, height = 6.2)
ggsave(file.path(base,"DE_analysis","A549_volcano.png"), p_final, width = 6.6, height = 6.2, dpi = 300)

write.csv(labs_df %>% select(gene, SYMBOL, log2FoldChange, padj),
          file.path(base,"DE_analysis","A549_volcano_labels.csv"), row.names = FALSE)
