...
base <- "/gpfs01/home/alysl56/projects/DMSO_HepG2_RNAseq"
f <- file.path(base,"DE_analysis","HepG2_DESeq2_results.csv")
...
p_core <- ggplot(df, aes(x = log2FoldChange, y = neglog10padj, color = status)) +
  ...
  labs(x = expression(log[2]*"Fold Change"),
       ...
       title    = "Volcano plot",
       subtitle = "HepG2: with DMSO vs without DMSO") +
...
ggsave(file.path(base,"DE_analysis","HepG2_volcano.pdf"), p_final, width = 6.6, height = 6.2)
ggsave(file.path(base,"DE_analysis","HepG2_volcano.png"), p_final, width = 6.6, height = 6.2, dpi = 300)

write.csv(dplyr::select(labs_df, gene, SYMBOL, log2FoldChange, padj),
          file.path(base,"DE_analysis","HepG2_volcano_labels.csv"), row.names = FALSE)
